...
def main(ctx, input, input_refps, refps_round, output, temp_dir, ffmpeg_bin_dir, realcugan, realcugan_models, noise_level, rife, rife_models, gpu_id, j_threads, output_resolution, output_fps, log_level, log_file, if_exist, parallel, ffmpeg_threads):
    ...
    for input_file in input_files:
        ...
        if w_scale != h_scale:
            logging.warning("input %sx%s and output %s is not the same ratio, it might be a problem" % (
                input_res[0], input_res[1], output_resolution))
        ...
        conv_obj = converter(input_file).ffmpeg_v2p(
            target_fps=input_refps, round=refps_round, threads=ffmpeg_threads)
        if res_scale in (2, 3, 4):
            conv_obj.realcugan(scale=res_scale, noise=noise_level,
                               model=realcugan_models, gpu_id=gpu_id, j_threads=j_threads)
        elif res_scale in (5, 6):
            conv_obj.realcugan(scale=6, noise=noise_level,
                               model=realcugan_models, gpu_id=gpu_id, j_threads=j_threads)
        elif res_scale in (7, 8):
            conv_obj.realcugan(scale=8, noise=noise_level,
                               model=realcugan_models, gpu_id=gpu_id, j_threads=j_threads)
        elif res_scale == 1:
            pass
        else:
            logging.warning(
                "not support scale larger than 8, current scale %s, doing scale=8 anyway" % res_scale)
